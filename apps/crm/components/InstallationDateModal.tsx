...
interface InstallationDateModalProps {
...
export default function InstallationDateModal({
  project,
  onClose,
  onSuccess,
  onUpdateProject,
}: InstallationDateModalProps) {
  const [installationDate, setInstallationDate] = useState(
    project.installationDate || new Date().toISOString().split('T')[0]
  )
  const [installationTime, setInstallationTime] = useState(project.installationTime || '')
    ...
    try {
      ...
      try {
        await createAppointment({
          ...
          time: installationTime || undefined,
          type: 'Installation',
          notes: `Montage für Auftrag ${project.orderNumber}`,
        })
        ...
        if (errMessage.includes('aborted') || errName === 'AbortError') {
          ...
          logger.error('Error creating calendar appointment', { component: 'InstallationDateModal' }, appointmentError instanceof Error ? appointmentError : new Error(String(appointmentError)))
        }
      ...
      logger.error('Error saving installation date', { component: 'InstallationDateModal' }, error instanceof Error ? error : new Error(String(error)))
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/70 p-4 backdrop-blur-md">
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-white px-8 py-6">
          <div>
            <h2 className="text-2xl font-black text-slate-900">Montage-Datum</h2>
        ...
        <div className="space-y-6 p-8">
          ...
          <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
            <p className="text-sm text-blue-900">
              <strong>💡 Hinweis:</strong> Das Montage-Datum wird im Projekt gespeichert und
