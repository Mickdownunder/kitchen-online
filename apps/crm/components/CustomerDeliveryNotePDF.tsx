...
const CustomerDeliveryNotePDFDocument: React.FC<CustomerDeliveryNotePDFProps> = ({
  deliveryNote,
  project,
  company,
}) => {
  const companyFullName =
    (company?.companyName || 'Ihr Unternehmen') +
...
export const downloadCustomerDeliveryNotePDF = async (
  deliveryNote: CustomerDeliveryNotePDFProps['deliveryNote'],
  project: CustomerProject,
  company?: CompanySettings | null
): Promise<void> => {
  const blob = await pdf(
    <CustomerDeliveryNotePDFDocument
  ...
  link.href = url
  link.download = `Lieferschein_${deliveryNote.deliveryNoteNumber.replace(/\//g, '-')}_${project.customerName.replace(/\s/g, '_')}.pdf`
...
export default CustomerDeliveryNotePDFDocument
